...
class PhoneScheduleController extends Controller
{
    ...
    public function createArrayOfOpeningHours($rows): array
    {
        ...
        foreach ($rows as $row) {
            ...
            $duplicates = array_unique(array_diff_assoc(
                $openingHoursList[$row['name']],
                array_unique($openingHoursList[$row['name']])
            ));
            $openingHoursList[$row['name']] = array_diff($openingHoursList[$row['name']], $duplicates);
        }
