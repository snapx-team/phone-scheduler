                ...
                <div class="w-full grid sm:grid-cols-2 gap-3 sm:gap-3">
                    <div class="pt-4">
                        ...
                        <div class="flex bg-blue-50 rounded-lg p-4 my-3 text-sm text-blue-800" role="alert">
                            <div>
                                <div class="font-medium">English Intro</div>
                                <p class="pt-1">{{directoryData.messageEN}}</p>
                                <p>To speak to ***, press 1. To speak to *** press 2...</p>
                            </div>
                        </div>

                        <div class="flex bg-green-50 rounded-lg p-4 my-3 text-sm text-green-800" role="alert">
                            <div>
                                <div class="font-medium">English Opening Hours Text</div>
                                <p class="pt-1">{{directoryData.messageEN}}</p>
                                <p>{{openingHoursData.openingHoursEnText}}</p>
                            </div>
                        </div>
                    </div>
                    <div class="pt-4">
                        ...
                        <div class="flex bg-blue-50 rounded-lg p-4 my-3 text-sm text-blue-800" role="alert">
                            <div>
                                <div class="font-medium">French Intro</div>
                        ...
                        <div class="flex bg-green-50 rounded-lg p-4 my-3 text-sm text-green-800" role="alert">
                            <div>
                                <div class="font-medium">French Opening Hours Text</div>
                                <p class="pt-1">{{directoryData.messageFR}}</p>
                                <p>{{openingHoursData.openingHoursFrText}}</p>
                            </div>
                        </div>
                    </div>
                </div>
...
export default {
    ...
    methods: {
            ...
            switch (eventName) {
                case "moved":
                    this.asyncUpdateEmployeeCardIndexes(employeeCardData).then(() => {
                        this.getOpeningHours(this.kanban.id);
                        this.isDraggableDisabled = false
                    });
                    break;
                case "added":
        ...
        getOpeningHours(id){
            this.asyncGetOpeningHoursData(id).then((data) => {
                this.openingHoursData = data.data;
            });
        },
        ...
        getKanban(kanbanID) {
            ...
            this.asyncGetPhoneLineData(kanbanID).then((data) => {
                this.kanban = data.data.phoneLine;
                this.openingHoursData = data.data.openingHours;
